...
def app(patched_requests):
    ...
    dummy_app.test_plugin.initialize()
    return dummy_app


def test_jira(patched_requests, app):
...
def test_jira_multiple(patched_requests, app):
    # Test multiple issues in a single message
    clear_recent_issues(app)
    responses = app.respond("I think TEST-123 and TEST-234 are related")
    expected_calls = [
        call('https://tickets.test.org/rest/api/2/project'),
        call('https://tickets.test.org/rest/api/2/issue/TEST-123'),
        call('https://tickets.test.org/rest/api/2/issue/TEST-234')
    ]
    ...
    assert patched_requests.mock_calls == expected_calls
    assert responses[0].split('\n') == expected_response
    clear_recent_issues(app)


def test_jira_cooldown(patched_requests, app):
...
def test_jira_unicode(patched_requests, app):
